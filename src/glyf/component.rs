...
mod tests {
    ...
    fn test_glyf_component() {
        let binary_glyph = vec![
            0xFF, 0xFF, 0x00, 0x32, 0xFE, 0x45, 0x02, 0xD2, 0x01, 0xD6, 0x02, 0x06, 0x00, 0x01,
            0x00, 0xFE,
        ];
        let deserialized: Glyph = otspec::de::from_bytes(&binary_glyph).unwrap();
        assert_eq!(
            deserialized.components[0].transformation,
            Affine::new([
            1.0,
            0.0,
            0.0,
