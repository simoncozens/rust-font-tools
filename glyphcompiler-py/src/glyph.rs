...
#[derive(Default)]
pub(crate) struct GlyphForConversion<'a> {
    pub(crate) masters: Vec<Option<UnconvertedMaster<'a>>>,
    pub(crate) default_master_ix: usize,
    pub(crate) model: Option<&'a VariationModel<String>>,
    /// The glyph's name, for debugging purposes
    pub(crate) glif_name: &'a str,
}
...
impl<'a> GlyphForConversion<'a> {
    ...
    pub(crate) fn convert(mut self) -> GlyphReadyToGo<'a> {
        assert!(
...
pub(crate) struct UnconvertedMaster<'a> {
    pub(crate) babelfont_contours: Vec<&'a babelfont::Path>,
    pub(crate) components: Vec<glyf::Component>,
    pub(crate) width: i32,
}

#[derive(Default)]
pub(crate) struct GlyphReadyToGo<'a> {
    pub(crate) masters: Vec<Option<ConvertedMaster>>,
...
#[derive(Default)]
pub(crate) struct ConvertedMaster {
    pub(crate) glyf_contours: GlyphContour,
    pub(crate) components: Vec<glyf::Component>,
    pub(crate) width: i32,
}
...
pub(crate) fn babelfont_component_to_glyf_component(
